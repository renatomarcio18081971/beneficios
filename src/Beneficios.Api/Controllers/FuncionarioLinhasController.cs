...
public class FuncionarioLinhasController : ControllerBase
{
        ...
        [FromQuery] bool? somenteVigentes)
    {
        ...
        {
            _logger.LogError(ex, "Erro ao listar vínculos funcionário × linhas");
            return StatusCode(500, new { message = "Erro ao listar vínculos funcionário × linhas" });
        }
    }
    ...
    public async Task<IActionResult> ObterPorId(Guid id)
    {
        ...
        {
            _logger.LogError(ex, "Erro ao obter vínculo funcionário × linha {Id}", id);
            return StatusCode(500, new { message = "Erro ao obter vínculo funcionário × linha" });
        }
    }
    ...
    public async Task<IActionResult> Criar([FromBody] FuncionarioLinhaSalvarDto dto)
    {
        ...
        {
            _logger.LogError(ex, "Erro ao criar vínculo funcionário × linha");
            return StatusCode(500, new { message = "Erro ao criar vínculo funcionário × linha" });
        }
    }
    ...
    public async Task<IActionResult> Atualizar(Guid id, [FromBody] FuncionarioLinhaAtualizarDto dto)
    {
        ...
        {
            _logger.LogError(ex, "Erro ao atualizar vínculo funcionário × linha {Id}", id);
            return StatusCode(500, new { message = "Erro ao atualizar vínculo funcionário × linha" });
        }
    }
