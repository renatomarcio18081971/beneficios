-- Migração idempotente de schemas tenant_* para o modelo de perfis.
-- Preferir chamar ITenantProvisioner.EnsurePerfisEmTenantsExistentesAsync em runtime.
-- Este script documenta o equivalente manual por schema ({schema}).
...
-- Em seguida: inserir perfil Dono (eh_sistema) + permissões do catálogo se ausente,
-- e UPDATE usuarios SET perfil_id = dono WHERE email = 'user@123.com' AND perfil_id IS NULL.
